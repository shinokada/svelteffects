<script lang="ts">
  ...
  let { children, bgColor = '#ebe7e0', padding = '50', size = '6vw', shadowStyle = 'elegant', opacity = 1, style }: Props = $props();
</script>

<div class={shadowStyle} style="padding: {padding}px; background-color: {bgColor};  opacity: {opacity}; font-size: {size}; line-height: 1.3; text-rendering: optimizeLegibility; {style}">
  {@render children()}
</div>
