...
<a {href} class={underlineStyle} style={Object.entries(linkStyle).map(([key, value]) => `${key}:${value}`).join(';')}>
  {@render children()}
</a>
...
<style>
  ...
  a::after {
    ...
    transition: opacity var(--underline-duration), transform var(--underline-duration);
  }
